...
public class Environment extends javax.swing.JFrame {
    ...
    private void RunActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RunActionPerformed
        ...
        while (clock < duration) {
            ...
            for (Iterator<Sensor> it = transmitting.iterator(); it.hasNext();) {
                ...
                int x = s_x.get(sensor);
                int y = s_y.get(sensor);
                int c = sensor.getColor();
                int r = sensor.getRange();
                for (int i = max(x - r, 0); i < min(x + r + 1, width); i++) {
                    for (int j = max(y - r, 0); j < min(y + r + 1, length); j++) {
                        if ((int) (Math.sqrt((x - i) * (x - i) + (y - j) * (y - j))) <= r
                                && board[i][j] != null && board[i][j].getState() == c + 1) {
                            //System.out.println("INCREASED!");
                            if (c == 0) {
                                board[i][j].add_red_sensor(sensor.getNumber());
                            } else if (c == 1) {
                                board[i][j].add_blue_sensor(sensor.getNumber());
                            }
                        }
                    }
                }
